...
import { LayoutDashboard, Compass, CheckCircle2, Database, ShieldCheck } from 'lucide-react';

const links = [
  { name: 'Index Overview', path: '/', icon: LayoutDashboard },
    ...
    <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col justify-between p-4">
      <div>
        <div className="flex items-center gap-3 px-2 py-4 border-b border-slate-800">
          ...
          <div>
            <div className="font-bold text-slate-100 tracking-tight">APIx Engine</div>
            <div className="text-xs text-blue-400 font-mono">MoSPI / NSO Edition</div>
          </div>
        </div>
      ...
      <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/80 text-xs text-slate-400 space-y-2">
        ...
        <div>Formula: Jevons Geometric Mean (Micro) + Laspeyres (Macro $W_r$)</div>
        <div className="text-[10px] text-slate-500">COICOP: 07.3.3 Passenger Transport</div>
      </div>
    </aside>
  );
}